...
static GParamSpec* (*fp_g_param_spec_int)(const gchar *name, const gchar *nick,
 ...
 ************************/
static GdkPixmap *(*fp_gdk_pixmap_new)(GdkDrawable *drawable, gint width,
		gint height, gint depth);
static GdkGC *(*fp_gdk_gc_new)(GdkDrawable*);
static void (*fp_gdk_rgb_gc_set_foreground)(GdkGC*, guint32);
static void (*fp_gdk_draw_rectangle)(GdkDrawable*, GdkGC*, gboolean, gint,
		gint, gint, gint);
...
static GdkPixbuf *(*fp_gdk_pixbuf_get_from_drawable)(GdkPixbuf *dest,
		GdkDrawable *src, GdkColormap *cmap, int src_x, int src_y, int dest_x,
		int dest_y, int width, int height);
static void (*fp_gdk_drawable_get_size)(GdkDrawable *drawable, gint* width,
		gint* height);
...
static void (*fp_gtk_paint_shadow)(GtkStyle* style, GdkWindow* window,
		...
		GtkWidget* widget, const gchar* detail, gint x, gint y, gint width,
		gint height);
static void (*fp_gtk_paint_arrow)(GtkStyle* style, GdkWindow* window,
		GtkStateType state_type, GtkShadowType shadow_type, GdkRectangle* area,
		GtkWidget* widget, const gchar* detail, GtkArrowType arrow_type,
...
static GtkWidget* (*fp_gtk_progress_bar_set_orientation)(GtkProgressBar *pbar,
...
static GtkObject* (*fp_gtk_adjustment_new)(gdouble value, gdouble lower,
...
static void (*fp_gtk_arrow_set)(GtkWidget* arrow, GtkArrowType arrow_type,
		GtkShadowType shadow_type);
...
gboolean gtk2_check_version_args(guint required_major, guint required_minor,
	...
	if (gtk2_libhandle != NULL) {
		...
		if (!fp_gtk_check_version(required_major, required_minor,
				required_micro)) {
			result = TRUE;
		}
...
gboolean gtk2_load() {
	...
	if (setjmp(j) == 0) {
		...
		fp_g_main_context_iteration = dl_symbol("g_main_context_iteration");
		...
		fp_gdk_pixbuf_get_from_drawable = dl_symbol(
				"gdk_pixbuf_get_from_drawable");
		fp_gdk_gc_new = dl_symbol("gdk_gc_new");
		fp_gdk_rgb_gc_set_foreground = dl_symbol("gdk_rgb_gc_set_foreground");
		...
		fp_gdk_pixbuf_get_bits_per_sample = dl_symbol(
		...
		fp_gtk_style_apply_default_background = dl_symbol(
		...
		fp_gtk_color_selection_dialog_new = dl_symbol(
		...
		fp_gtk_progress_bar_set_orientation = dl_symbol(
		...
		fp_gtk_separator_menu_item_new = dl_symbol(
		...
		fp_gtk_widget_class_install_style_property = dl_symbol(
				"gtk_widget_class_install_style_property");
		fp_gtk_widget_class_find_style_property = dl_symbol(
				"gtk_widget_class_find_style_property");
		fp_gtk_widget_style_get_property = dl_symbol(
				"gtk_widget_style_get_property");
		fp_pango_font_description_to_string = dl_symbol(
		...
		fp_gtk_combo_box_entry_new = dlsym(gtk2_libhandle,
		...
		fp_gtk_separator_tool_item_new = dlsym(gtk2_libhandle,
				"gtk_separator_tool_item_new");
		if (fp_gtk_separator_tool_item_new == NULL) {
			fp_gtk_separator_tool_item_new = dl_symbol("gtk_vseparator_new");
		}
	}
	...
	if (gtk_modules_env && strstr(gtk_modules_env, "atk-bridge")
		...
		gchar *s, *new_env = malloc(sizeof(ENV_PREFIX)
				+ strlen(gtk_modules_env));
...
int gtk2_unload() {
	...
	if (gtk2_white_pixmap != NULL) {
		...
		gtk2_white_pixmap = gtk2_black_pixmap = gtk2_white_pixbuf
				= gtk2_black_pixbuf = NULL;
	}
	gtk2_pixbuf_width = 0;
...
void gtk2_init_painting(gint width, gint height) {
	...
	if (gtk2_pixbuf_width < width || gtk2_pixbuf_height < height) {
		...
		gtk2_white_pixmap = (*fp_gdk_pixmap_new)(gtk2_window->window, width,
				height, -1);
		gtk2_black_pixmap = (*fp_gdk_pixmap_new)(gtk2_window->window, width,
				height, -1);
...
static void gtk2_set_direction(GtkWidget *widget, GtkTextDirection dir) {
	/*
...
static void init_toggle_widget(WidgetType widget_type, gint synth_state) {
	...
	if (widget_type == RADIO_BUTTON || widget_type == CHECK_BOX || widget_type
			== TOGGLE_BUTTON) {
...
static GtkWidget* gtk2_get_arrow(GtkArrowType arrow_type,
		GtkShadowType shadow_type) {
	GtkWidget *arrow = NULL;
	if (NULL == gtk2_widgets[_GTK_ARROW_TYPE]) {
		gtk2_widgets[_GTK_ARROW_TYPE] = (*fp_gtk_arrow_new)(arrow_type,
				shadow_type);
		(*fp_gtk_container_add)((GtkContainer *) gtk2_fixed,
				gtk2_widgets[_GTK_ARROW_TYPE]);
...
static GtkAdjustment* create_adjustment() {
	return (GtkAdjustment *) (*fp_gtk_adjustment_new)(50.0, 0.0, 100.0, 10.0,
			20.0, 20.0);
}
...
static GtkWidget *gtk2_get_widget(WidgetType widget_type) {
	...
	case CHECK_BOX:
		if (init_result = (NULL == gtk2_widgets[_GTK_CHECK_BUTTON_TYPE])) {
			gtk2_widgets[_GTK_CHECK_BUTTON_TYPE] = (*fp_gtk_check_button_new)();
		}
	...
	case COMBO_BOX:
		if (init_result = (NULL == gtk2_widgets[_GTK_COMBO_BOX_TYPE])) {
			gtk2_widgets[_GTK_COMBO_BOX_TYPE] = (*fp_gtk_combo_box_new)();
		}
	...
	case LABEL:
		if (init_result = (NULL == gtk2_widgets[_GTK_LABEL_TYPE])) {
			gtk2_widgets[_GTK_LABEL_TYPE] = (*fp_gtk_label_new)(NULL);
		}
	...
	case ROOT_PANE:
		if (init_result = (NULL == gtk2_widgets[_GTK_CONTAINER_TYPE])) {
			 ...
			 * constructor.
			 */
			gtk2_widgets[_GTK_CONTAINER_TYPE] = (*fp_gtk_fixed_new)();
		}
	...
	case TEXT_PANE:
		if (init_result = (NULL == gtk2_widgets[_GTK_TEXT_VIEW_TYPE])) {
			gtk2_widgets[_GTK_TEXT_VIEW_TYPE] = (*fp_gtk_text_view_new)();
		}
	...
	case TEXT_FIELD:
		if (init_result = (NULL == gtk2_widgets[_GTK_ENTRY_TYPE])) {
			gtk2_widgets[_GTK_ENTRY_TYPE] = (*fp_gtk_entry_new)();

			GtkSettings* settings = fp_gtk_widget_get_settings(
	...
	case HANDLE_BOX:
		if (init_result = (NULL == gtk2_widgets[_GTK_HANDLE_BOX_TYPE])) {
			gtk2_widgets[_GTK_HANDLE_BOX_TYPE] = (*fp_gtk_handle_box_new)();
		}
	...
	case HSCROLL_BAR_THUMB:
		if (init_result = (NULL == gtk2_widgets[_GTK_HSCROLLBAR_TYPE])) {
			gtk2_widgets[_GTK_HSCROLLBAR_TYPE] = (*fp_gtk_hscrollbar_new)(
					create_adjustment());
		}
	...
	case HSEPARATOR:
		if (init_result = (NULL == gtk2_widgets[_GTK_HSEPARATOR_TYPE])) {
			gtk2_widgets[_GTK_HSEPARATOR_TYPE] = (*fp_gtk_hseparator_new)();
		}
	...
	case HSLIDER_TRACK:
		if (init_result = (NULL == gtk2_widgets[_GTK_HSCALE_TYPE])) {
			gtk2_widgets[_GTK_HSCALE_TYPE] = (*fp_gtk_hscale_new)(NULL);
		}
	...
	case INTERNAL_FRAME:
		if (init_result = (NULL == gtk2_widgets[_GTK_WINDOW_TYPE])) {
			gtk2_widgets[_GTK_WINDOW_TYPE] = (*fp_gtk_window_new)(
					GTK_WINDOW_TOPLEVEL);
		}
	...
	case TREE_CELL:
		if (init_result = (NULL == gtk2_widgets[_GTK_TREE_VIEW_TYPE])) {
			gtk2_widgets[_GTK_TREE_VIEW_TYPE] = (*fp_gtk_tree_view_new)();
		}
	...
	case POPUP_MENU:
		if (init_result = (NULL == gtk2_widgets[_GTK_MENU_TYPE])) {
			gtk2_widgets[_GTK_MENU_TYPE] = (*fp_gtk_menu_new)();
		}
	...
	case MENU_ITEM_ACCELERATOR:
		if (init_result = (NULL == gtk2_widgets[_GTK_MENU_ITEM_TYPE])) {
			gtk2_widgets[_GTK_MENU_ITEM_TYPE] = (*fp_gtk_menu_item_new)();
		}
		result = gtk2_widgets[_GTK_MENU_ITEM_TYPE];
		break;
	case MENU_BAR:
		if (init_result = (NULL == gtk2_widgets[_GTK_MENU_BAR_TYPE])) {
			gtk2_widgets[_GTK_MENU_BAR_TYPE] = (*fp_gtk_menu_bar_new)();
		}
	...
	case OPTION_PANE:
		if (init_result = (NULL == gtk2_widgets[_GTK_DIALOG_TYPE])) {
			gtk2_widgets[_GTK_DIALOG_TYPE] = (*fp_gtk_dialog_new)();
		}
		result = gtk2_widgets[_GTK_DIALOG_TYPE];
		break;
	case POPUP_MENU_SEPARATOR:
		if (init_result = (NULL == gtk2_widgets[_GTK_SEPARATOR_MENU_ITEM_TYPE])) {
	...
	case RADIO_BUTTON:
		if (init_result = (NULL == gtk2_widgets[_GTK_RADIO_BUTTON_TYPE])) {
			gtk2_widgets[_GTK_RADIO_BUTTON_TYPE] = (*fp_gtk_radio_button_new)(
					NULL);
		}
		result = gtk2_widgets[_GTK_RADIO_BUTTON_TYPE];
		break;
	case RADIO_BUTTON_MENU_ITEM:
		if (init_result = (NULL == gtk2_widgets[_GTK_RADIO_MENU_ITEM_TYPE])) {
		...
		break;
	case SCROLL_PANE:
		if (init_result = (NULL == gtk2_widgets[_GTK_SCROLLED_WINDOW_TYPE])) {
	...
	case TABBED_PANE_TAB:
		if (init_result = (NULL == gtk2_widgets[_GTK_NOTEBOOK_TYPE])) {
			gtk2_widgets[_GTK_NOTEBOOK_TYPE] = (*fp_gtk_notebook_new)(NULL);
		}
	...
	case TOOL_BAR_DRAG_WINDOW:
		if (init_result = (NULL == gtk2_widgets[_GTK_TOOLBAR_TYPE])) {
			gtk2_widgets[_GTK_TOOLBAR_TYPE] = (*fp_gtk_toolbar_new)(NULL);
		}
		result = gtk2_widgets[_GTK_TOOLBAR_TYPE];
		break;
	case TOOL_BAR_SEPARATOR:
		if (init_result = (NULL == gtk2_widgets[_GTK_SEPARATOR_TOOL_ITEM_TYPE])) {
	...
	case VIEWPORT:
		if (init_result = (NULL == gtk2_widgets[_GTK_VIEWPORT_TYPE])) {
			...
			gtk2_widgets[_GTK_VIEWPORT_TYPE] = (*fp_gtk_viewport_new)(
	...
	case VSLIDER_TRACK:
		if (init_result = (NULL == gtk2_widgets[_GTK_VSCALE_TYPE])) {
			gtk2_widgets[_GTK_VSCALE_TYPE] = (*fp_gtk_vscale_new)(NULL);
		}
	...
	if (result != NULL && init_result) {
		if (widget_type == RADIO_BUTTON_MENU_ITEM || widget_type
				== CHECK_BOX_MENU_ITEM || widget_type == MENU_ITEM
				|| widget_type == MENU || widget_type == POPUP_MENU_SEPARATOR) {
		...
		} else if (widget_type == COMBO_BOX_ARROW_BUTTON || widget_type
		...
		} else if (widget_type != TOOL_TIP && widget_type != INTERNAL_FRAME
...
void gtk2_paint_arrow(WidgetType widget_type, GtkStateType state_type,
		GtkShadowType shadow_type, const gchar *detail, gint x, gint y,
		gint width, gint height, GtkArrowType arrow_type, gboolean fill) {
	static int w, h;
	...
	(*fp_gtk_paint_arrow)(gtk2_widget->style, gtk2_white_pixmap, state_type,
			shadow_type, NULL, gtk2_widget, detail, arrow_type, fill, x, y, w,
			h);
...
void gtk2_paint_box(WidgetType widget_type, GtkStateType state_type,
		GtkShadowType shadow_type, const gchar *detail, gint x, gint y,
		gint width, gint height, gint synth_state, GtkTextDirection dir) {
	gtk2_widget = gtk2_get_widget(widget_type);
...
void gtk2_paint_box_gap(WidgetType widget_type, GtkStateType state_type,
	...
	(*fp_gtk_paint_box_gap)(gtk2_widget->style, gtk2_white_pixmap, state_type,
			shadow_type, &area, gtk2_widget, detail, x, y, width, height,
...
void gtk2_paint_expander(WidgetType widget_type, GtkStateType state_type,
	...
	gtk2_widget = gtk2_get_widget(widget_type);
	(*fp_gtk_paint_expander)(gtk2_widget->style, gtk2_white_pixmap, state_type,
			NULL, gtk2_widget, detail, x + width / 2, y + height / 2,
			expander_style);
	(*fp_gtk_paint_expander)(gtk2_widget->style, gtk2_black_pixmap, state_type,
			NULL, gtk2_widget, detail, x + width / 2, y + height / 2,
			expander_style);
}
...
void gtk2_paint_extension(WidgetType widget_type, GtkStateType state_type,
	...
	(*fp_gtk_paint_extension)(gtk2_widget->style, gtk2_white_pixmap,
			state_type, shadow_type, NULL, gtk2_widget, detail, x, y, width,
			height, gap_side);
...
void gtk2_paint_flat_box(WidgetType widget_type, GtkStateType state_type,
		GtkShadowType shadow_type, const gchar *detail, gint x, gint y,
		gint width, gint height, gboolean has_focus) {
	gtk2_widget = gtk2_get_widget(widget_type);
	...
	(*fp_gtk_paint_flat_box)(gtk2_widget->style, gtk2_white_pixmap, state_type,
			shadow_type, NULL, gtk2_widget, detail, x, y, width, height);
	(*fp_gtk_paint_flat_box)(gtk2_widget->style, gtk2_black_pixmap, state_type,
			shadow_type, NULL, gtk2_widget, detail, x, y, width, height);
}
...
gint gtk2_get_color_for_state(JNIEnv *env, WidgetType widget_type,
	...
	if (color)
		result = recode_color(color->red) << 16 | recode_color(color->green)
				<< 8 | recode_color(color->blue);
...
void gtk2_set_range_value(WidgetType widget_type, jdouble value, jdouble min,
		jdouble max, jdouble visible) {
	GtkAdjustment *adj;
...
/*************************************************/
jobject create_Object(JNIEnv *env, jmethodID *cid, const char* class_name,
		const char* signature, jvalue* value) {
	jclass class;
	jobject result;
...
/***********************************************/
jobject get_string_property(JNIEnv *env, GtkSettings* settings,
		const gchar* key) {
	jobject result = NULL;
	gchar* strval = NULL;
